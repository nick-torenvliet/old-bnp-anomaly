...
def run_bnp_anomaly(mppack):
    ...
    iname='randexamples'
    opath = f'/tmp/bnp-temp/coldstart/{G}/{ds}/b0'  # Dynamic output path according to batch
    ...
    for ii, window in enumerate(windows):
        ...
        opath = f'/tmp/bnp-temp/warmstart/{G}/{ds}/b{ii +  1}'
...
for d, df in enumerate(data):
    ...
    while i * batch_size <= (len(df) - window_size_in_batches * batch_size):
        ...
        y = w.drop(columns=['timestamp'])
        yy = bnpy.data.XData.from_dataframe(y)
        x = w.index
        win.append([x, yy])
        i += 1
    windows.append([win, batch_size, window_size_in_batches, test_data_names[d], 0])
    windows.append([win, batch_size, window_size_in_batches, test_data_names[d], 2])
