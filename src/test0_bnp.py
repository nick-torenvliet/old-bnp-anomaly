...
def run_bnp_anomaly(mppack):
    ...
    nLap = 20
    iname='randexamples'
    opath = f'/tmp/bnp-anomaly/coldstart/0{data_set}/b0'  # Dynamic output path according to batch
    ...
    for ii, window in enumerate(windows):
        ...
        warm_start_model, warm_info_dict = bnpy.run(
            ...
            initname=iname,
            ts=True, debug=False, verbose=0, G=0)
        
        iname=warm_info_dict['task_output_path']
        opath = f'/tmp/bnp-anomaly/warmstart/0{data_set}/b{ii +  1}'
...
data = [pd.read_csv(i, usecols=['0']).head(4000) for i in data_sets]
